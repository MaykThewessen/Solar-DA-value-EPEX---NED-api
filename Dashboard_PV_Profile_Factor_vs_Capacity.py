...
if len(plot_data) > 2:
    ...
    # Add linear trend line extending to September 11, 2025
    ...
    # Create trend line extending from 100% to January 1, 2026
    # Find x-value where y = 100% using linear equation: x = (100 - b) / a
    x_at_100_percent = (100 - coeffs_linear[1]) / coeffs_linear[0]
    x_extended_linear = np.linspace(x_at_100_percent, jan_1_capacity, 50)
    trend_line_extended_linear = np.polyval(coeffs_linear, x_extended_linear)
    
    # Add solid linear trend line extending to September 11, 2025
    fig.add_trace(
        go.Scatter(
            x=x_extended_linear,
            y=trend_line_extended_linear,
            mode='lines',
            name='Linear Trend (solid)',
            line=dict(color='red', width=2),
            hovertemplate='<b>Linear Trend (from 100% to Jan 1, 2026)</b><br>' +
                         f'<b>Equation:</b> y = {coeffs_linear[0]:.2f}x + {coeffs_linear[1]:.2f}<br>' +
                         f'<b>Slope:</b> {coeffs_linear[0]:.2f} %/GWp<br>' +
                         f'<b>Starts at:</b> {x_at_100_percent:.1f} GWp (100%)<br>' +
        ...
        if b < 0:  # Only for decay (negative slope)
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=y_extrapolation,
                    mode='lines',
                    name='Asymptotic Extrapolation (striped)',
                    line=dict(color='orange', width=2, dash='dash'),
                    hovertemplate='<b>Exponential Extrapolation (from Jan 1, 2026)</b><br>' +
                                 '<b>Capacity:</b> %{x:.1f} GWp<br>' +
                                 '<b>Profile Factor:</b> %{y:.1f}%<br>' +
                                 '<b>Equation:</b> y = ' + f'{a:.2f} * e^({b:.3f}*x)<br>' +
                                 '<b>Starts from:</b> {jan_1_capacity:.1f} GWp (Jan 1, 2026)<br>' +
                                 '<extra></extra>',
                    showlegend=True
                )
            )
            ...
            for i, threshold in enumerate(thresholds):
                if threshold < a:  # Only show if threshold is below the initial value
            ...
            fig.add_annotation(
                ...
                text='Exponential trend at 55 GWp: 24%',
                showarrow=True,
                arrowhead=2,
                arrowsize=1,
                arrowwidth=2,
...
if len(plot_data) > 2:
    ...
    # Print linear trend (from 100% to January 1, 2026)
    print(f"Linear trend equation (from 100% to Jan 1, 2026): y = {coeffs_linear[0]:.2f}x + {coeffs_linear[1]:.2f}")
    print(f"Linear trend starts at: {x_at_100_percent:.1f} GWp (100%)")
    ...
    if 'a' in locals() and 'b' in locals():
        print(f"Exponential extrapolation equation: y = {a:.2f} * e^({b:.3f}*x)")
