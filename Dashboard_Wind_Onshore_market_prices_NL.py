...
fig.add_trace(
    go.Table(
        header=dict(
            values=['Year (* = preliminary)', 'Installed Onshore Wind Capacity in NL (GW) mid-year', 'Onshore Wind Energy produced (TWh/y) (NED.nl)', 'MWh yield / MW installed', 'MWh yield / MW (excl. neg)', 'Curtailment (%)', 'Negative-price hours (h/y)', 'Annual Market value (EUR/MW/y)', 'Market value EUR/MW/y (excl. neg)', 'Day-Ahead linear avg price (EUR/MWh)', 'Onshore wind capture price (€/MWh)', 'Onshore wind capture price (€/MWh) excl. neg', 'Onshore wind capture rate (%)', 'Onshore wind capture rate (%) excl. neg'],
            font=dict(size=10),
            align='left'
        ),
        cells=dict(
...
fig.update_yaxes(title_text='Onshore wind capture rate (%)', row=5, col=1, range=[0, 100])
...
fig.update_yaxes(title_text='Onshore wind capture price (€/MWh)', row=6, col=1)
...
table_fig = go.Figure(data=[go.Table(
    header=dict(
        values=['Month', 'Onshore Wind generation capacity NL (GW AC)', 'Onshore Wind Energy produced (GWh/month) (NED.nl)', 'MWh yield / MW (excl. neg)', 'Curtailment (%)', 'Negative-price hours (h)', 'Market value Onshore Wind (EUR/MW/year)', 'Market value EUR/MW/y (excl. neg)', 'Day-Ahead average price (EUR/MWh)', 'Onshore wind capture price (€/MWh)', 'Onshore wind capture price (€/MWh) excl. neg', 'Onshore wind capture rate (%)', 'Onshore wind capture rate (%) excl. neg'],
        font=dict(size=10),
        align='left'
    ),
            cells=dict(
...
table_fig.write_html('wind_onshore_monthly_summary_table.html', auto_open=False)
fig.write_html('wind_onshore_production_plot_v3.html', auto_open=False)
...
_pdf_slides = [
    dict(
        title='Installed Onshore Wind Capacity', subtitle='Netherlands · year-end GW AC (Birdview Central scenario)',
        ytitle='GW (AC)', kind='area_gradient',
        traces=[(yst['Yearly_Installed_Capacity_GW_AC'], 'Installed capacity', CLAUDE_PALETTE['accent'])],
        mask_partial=False,
    ),
    dict(
        title='Onshore Wind Energy Produced', subtitle='Annual generation in TWh · complete years only · NED.nl',
    ...
    dict(
        ...
        subtitle='Volume-weighted onshore-wind price compared with flat Day-Ahead average',
    ...
    dict(
        title='Onshore Wind Capture Rate',
...
_monthly_fig = build_monthly_metric_by_year_fig(
    ...
    title='Monthly Onshore Wind Capture Rate',
...
_table_fig = build_yearly_summary_table_fig(
    ...
    title='Yearly Onshore Wind Market Summary',
...
render_slides_to_pdf(_slide_pages, 'wind_onshore_yearly_slides.pdf')
print('PDF written: wind_onshore_yearly_slides.pdf')
