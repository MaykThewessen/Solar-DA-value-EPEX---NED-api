...
fig = make_subplots(
    ...
    subplot_titles=(
        'Monthly PV Yield per MW Installed',
        'Monthly PV Value per MW and Monthly PV Power Weighted DA Price (EUR/MWh)'
    ),
    specs=[[{"secondary_y": False}], [{"secondary_y": True}]]
)

# First subplot: Monthly PV Yield per MW (bar)
fig.add_trace(
    go.Bar(x=monthly['month_date'], y=monthly['Monthly_PV_Yield_per_MW'], name='Monthly PV Yield MWh/MWp', marker_color='green', opacity=0.9),
...
fig.add_trace(
    go.Bar(x=monthly['month_date'], y=monthly['Monthly_Value_per_MWp_DC_EUR'], name='Monthly PV Value (EUR/MWp)', marker_color='goldenrod', opacity=0.8),
    row=2, col=1, secondary_y=False
)
fig.add_trace(
    go.Bar(x=monthly['month_date'], y=monthly['Avg_DA_Price'] if 'Avg_DA_Price' in monthly else monthly_summary['Avg_DA_Price'], name='Monthly Avg DA Price (EUR/MWh)', marker_color='deepskyblue', opacity=0.6),
    row=2, col=1, secondary_y=True
)
fig.add_trace(
    go.Bar(x=monthly['month_date'], y=monthly['Monthly_PV_Power_Weighted_DA_Price'], name='Monthly PV Power Weighted DA Price (EUR/MWh)', marker_color='purple', opacity=0.7),
    row=2, col=1, secondary_y=True
)
...
fig.update_layout(
    ...
    title_text='Monthly PV Yield, Value, and Weighted Price',
