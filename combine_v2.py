...
yearly_totals = (
    df_combined.groupby('year').apply(
        lambda x: pd.Series({
            'Total_PV_Energy_MWh': x['Solar_production_MWh'].sum(),
            ...
            'Yearly_Yield_per_MWp_MWh': x['Solar_production_MWh'].sum() / x['installed_capacity_MW'].mean(),
            'Yearly_Weighted_Price_EUR_MWh': (x['Solar_production_MWh'] * x['DA_price']).sum() / x['Solar_production_MWh'].sum() if x['Solar_production_MWh'].sum() > 0 else float('nan')
        })
    )
    .reset_index()
)
