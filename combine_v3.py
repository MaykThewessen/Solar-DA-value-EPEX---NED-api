...
yearly_totals = monthly.groupby('year').agg({
    ...
    'Monthly_Value_per_MWp_DC_EUR': lambda x: (x * monthly.loc[x.index, 'Monthly_Installed_Capacity_MW']).sum() / monthly.loc[x.index, 'Monthly_Installed_Capacity_MW'].sum()
}).reset_index()
yearly_totals.columns = ['year', 'Yearly_PV_Energy_MWh', 'Yearly_Value_per_MWp_DC_EUR']
...
# --- Create subplots ---
fig = make_subplots(
    rows=3, cols=2, shared_xaxes=False, vertical_spacing=0.10, horizontal_spacing=0.15,
    subplot_titles=(
        'Monthly PV Yield', 'Monthly Values',
        'Market Value per MWp installed', 'Yearly Summary',
        'PV Weighted DA Price & Profile Factor', 'Value per MWh'
    ),
    specs=[
