...
# Graph shows yearly relationship between installed Onshore Wind capacity and profile factor
...
yearly_totals = yearly_totals.rename(columns={
    'time': 'year',
    'Wind_production_MWh': 'Yearly_Wind_Energy_MWh',
...
fig.update_layout(
    title=dict(
        text='Onshore Wind Profile Factor vs Installed Onshore Wind Capacity in Netherlands<br><sub>Each point represents year avg installed capacity vs yearly Onshore Wind profile factor</sub>',
        x=0.5,
        font=dict(size=16)
    ),
    xaxis=dict(
        title=dict(text='Installed Onshore Wind Capacity NL (GW AC) yearly avg', font=dict(size=14)),
...
fig.write_html('wind_onshore_profile_factor_vs_capacity_dashboard.html', auto_open=True)
fig.write_image('wind_onshore_profile_factor_vs_capacity_dashboard.pdf', format='pdf')

print(f"\nDashboard created:")
print(f"  - HTML: wind_onshore_profile_factor_vs_capacity_dashboard.html")
print(f"  - PDF: wind_onshore_profile_factor_vs_capacity_dashboard.pdf")
...
if len(plot_data) > 2:
    ...
    if 'a' in locals() and 'b' in locals():
        ...
        print("Asymptotic behavior: Onshore Wind profile factor approaches 0% but never reaches it")
        print("Onshore Wind will always retain some market value on the day-ahead market")
